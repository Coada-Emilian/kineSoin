/**
 * @description Defines all Sequelize associations between the application's models.
 *
 * Rationale:
 * - Centralizes relationship logic so each model stays focused on its own structure
 *   while associations remain easy to audit, maintain, and extend.
 * - Ensures consistent foreign‑key behavior (CASCADE rules, aliases, through‑tables)
 *   across the entire domain: admins, therapists, patients, medics, prescriptions,
 *   appointments, messages, and insurance links.
 *
 * Notes:
 * - Uses explicit aliases to keep include() queries predictable and readable.
 * - Many‑to‑many and one‑to‑many relationships are defined in a single place to
 *   avoid scattered or conflicting association declarations.
 */
