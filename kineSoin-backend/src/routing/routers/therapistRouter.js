/**
 * @description Express router defining all therapist‑side API endpoints.
 *
 * Rationale:
 * - Centralizes every therapist‑facing route in one place, giving the backend a
 *   predictable and well‑structured entry point for authenticated therapist actions.
 * - Enforces strict access control through `authenticateTherapist`, ensuring only
 *   authorized therapists can manage appointments, patient data, and messaging.
 *
 * Notes:
 * - Uses `controllerWrapper` for consistent async error handling.
 * - Keeps the router focused on request wiring and middleware composition, while
 *   delegating business logic to dedicated controllers.
 * - Follows a clear RESTful layout, making future therapist features easy to add.
 */
