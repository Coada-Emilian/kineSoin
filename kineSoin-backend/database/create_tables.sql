/**
 * @description Sets up the core database schema for the physiotherapy management system.
 *
 * Key design decisions:
 * - UTF‑8 encoding ensures consistent handling of multilingual patient/therapist data.
 * - All schema changes run inside a transaction to avoid partial migrations.
 * - Tables are dropped before creation to guarantee a clean reset during development.
 *
 * Architecture rationale:
 * - Entities (patients, therapists, medics, afflictions, prescriptions, appointments)
 *   are normalized and linked through foreign keys to preserve referential integrity.
 * - Cascading rules are applied only where domain logic allows automatic cleanup
 *   (e.g., deleting an admin removes their managed entities).
 * - SET NULL is used instead of CASCADE for patient/therapist relationships to avoid
 *   accidental data loss when a user is removed.
 *
 * Business rules encoded in schema:
 * - Status fields (patient/therapist) enforce workflow states directly at the DB level.
 * - Unique constraints (emails, licence codes, insurance identifiers) prevent duplicate
 *   medical identities and ensure regulatory compliance.
 * - Message tables store timestamps server‑side to guarantee chronological accuracy.
 *
 * Security considerations:
 * - Passwords are stored as hashed strings; schema enforces non-null constraints.
 * - Sensitive relationships (prescriptions, appointments) require explicit foreign keys
 *   to prevent orphaned medical records.
 *
 * WARNING:
 * - This script DROPS existing tables. Safe for development; dangerous in production.
 */
